const DEFAULT_BASE = process.env.EXPO_PUBLIC_API_BASE_URL || 'http://localhost:5000';
...
async function apiFetch(path, { method='GET', body, headers } = {}) {
  ...
  if (!res.ok) { const err = new Error(json?.error || res.statusText || 'Request failed'); err.status = res.status; err.payload = json; throw err; }
  return json;
}
